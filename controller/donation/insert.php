...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO blood_donation (no_of_bottles, user_user_id, patient_patient_id,blood_type_id) VALUES (?, ?, ?,?)");
    $stmt->bind_param("ssss", $no_of_bottles, $user_user_id, $patient_patient_id,$blood_type_id);

    // Execute the query
    if ($stmt->execute()) {
        $message= "New record created successfully";
        header("Location: ../../view/add_donation.php?message=$message");
        die();
    } else {
        //echo "Error: " . $stmt->error;
    $message= "donation records could not be created";
        header("Location: ../../view/add_donation.php?message=$message");
    }
